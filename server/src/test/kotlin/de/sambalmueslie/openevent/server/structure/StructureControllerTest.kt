...
internal class StructureControllerTest(userRepository: UserRepository) : BaseControllerTest<Structure>(userRepository) {
	...
	fun `02 check getter endpoints`() {
		val root = callPost(baseUrl, StructureChangeRequest(ItemDescriptionUtil.getCreateRequest(), null, null, true), adminToken)
		val children = callPost(baseUrl, StructureChangeRequest(ItemDescriptionUtil.getCreateRequest(), null, root.id, true), adminToken)
	...
	fun `03 get update and delete as other - not allowed`() {
		...
		assertThrows(HttpClientResponseException::class.java) {
			callPost(baseUrl, StructureChangeRequest(ItemDescriptionUtil.getCreateRequest(), null, null, true), otherToken)
		}
		...
		// update as other
		assertThrows(HttpClientResponseException::class.java) {
			callPut("$baseUrl/${root.id}", StructureChangeRequest(ItemDescriptionUtil.getUpdateRequest(), null, null, true), otherToken)
		}
	...
	fun `04 create, read update and delete - admin`() {
		val createRequest = StructureChangeRequest(ItemDescriptionUtil.getCreateRequest(), null, null, true)
		val createStructure = callPost(baseUrl, createRequest, adminToken, Structure::class.java)
		val structure = Structure(createStructure.id, true, true, true, admin, ItemDescriptionUtil.getCreateDescription(createStructure.description.id), null)
		...
		val updateRequest = StructureChangeRequest(updateItem, null, null, true)
